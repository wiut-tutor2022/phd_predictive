...
training_set = dataset_train.iloc[:, 20:21].values
...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 20:21].values

dataset_total = pd.concat((dataset_train['HP'], dataset_test['HP']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted HP')
plt.title('Real vs Predicted HP')
plt.xlabel('Months')
plt.ylabel('HP')
plt.legend()
plt.savefig('lstm_HP.png', dpi=1080, format='png')
plt.show()