...
training_set = dataset_train.iloc[:, 21:22].values
...
mei_lstm_pred = model.fit(X_train,y_train,batch_size=20,epochs=60)

dataset_test = pd.read_csv('Processed_Input_Data_FTSE100_1985_21.csv', header=0, index_col=0)
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 21:22].values

dataset_total = pd.concat((dataset_train['PMI'], dataset_test['PMI']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted PMI')
plt.title('Real vs Predicted PMI')
plt.xlabel('Months')
plt.ylabel('PMI')
plt.legend()
plt.savefig('lstm_PMI.png', dpi=1080, format='png')
plt.show()