...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 8:9].values

dataset_total = pd.concat((dataset_train['EI'], dataset_test['EI']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted EI')
plt.title('Real vs Predicted EI')
plt.xlabel('Months')
plt.ylabel('EI')
plt.legend()
plt.savefig('lstm_EI.png', dpi=1080, format='png')
plt.show()