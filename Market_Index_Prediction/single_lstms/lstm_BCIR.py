...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 14:15].values

dataset_total = pd.concat((dataset_train['BCIR'], dataset_test['BCIR']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted BCIR')
plt.title('Real vs Predicted BCIR')
plt.xlabel('Months')
plt.ylabel('BCIR')
plt.legend()
plt.savefig('lstm_BCIR.png', dpi=1080, format='png')
plt.show()