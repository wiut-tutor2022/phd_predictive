...
mei_lstm_pred = model.fit(X_train,y_train,batch_size=10,epochs=100)

dataset_test = pd.read_csv('Processed_Input_Data_FTSE100_1985_21.csv', header=0, index_col=0)
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 13:14].values

dataset_total = pd.concat((dataset_train['BDIR'], dataset_test['BDIR']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted BDIR')
plt.title('Real vs Predicted BDIR')
plt.xlabel('Months')
plt.ylabel('BDIR')
plt.legend()
plt.savefig('lstm_BDIR.png', dpi=1080, format='png')
plt.show()