...
training_set = dataset_train.iloc[:, 17:18].values
...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 17:18].values

dataset_total = pd.concat((dataset_train['FDI'], dataset_test['FDI']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted FDI')
plt.title('Real vs Predicted FDI')
plt.xlabel('Months')
plt.ylabel('FDI')
plt.legend()
plt.savefig('lstm_FDI.png', dpi=1080, format='png')
plt.show()