...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 10:11].values

dataset_total = pd.concat((dataset_train['AWE'], dataset_test['AWE']), axis=0)
...
#it's called a predicted_stock_price for our convenience here, but actually it's the indicator's value
...
plt.plot(predicted_stock_price, color='green', label='Predicted AWE')
plt.title('Real vs Predicted AWE')
plt.xlabel('Months')
plt.ylabel('AWE')
plt.legend()
plt.savefig('lstm_AWE.png', dpi=1080, format='png')
plt.show()