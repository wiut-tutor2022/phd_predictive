...
training_set = dataset_train.iloc[:, 18:19].values
...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 18:19].values

dataset_total = pd.concat((dataset_train['IP'], dataset_test['IP']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted IP')
plt.title('Real vs Predicted IP')
plt.xlabel('Months')
plt.ylabel('IP')
plt.legend()
plt.savefig('lstm_IP.png', dpi=1080, format='png')
plt.show()