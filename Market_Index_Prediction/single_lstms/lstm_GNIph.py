...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 3:4].values

dataset_total = pd.concat((dataset_train['GNIph'], dataset_test['GNIph']), axis=0)
...
print('Predicted GNIph:')
# print(predicted_stock_price)
print(predicted_stock_price.transpose())

plt.rcParams["font.family"] = "Times New Roman"
plt.plot(real_stock_price, color='black', label='Real')
plt.plot(predicted_stock_price, color='green', label='Predicted GNIph')
plt.title('Real vs Predicted GNIph')
plt.xlabel('Months')
plt.ylabel('GNIph')
plt.legend()
plt.savefig('lstm_GNIph.png', dpi=1080, format='png')
plt.show()