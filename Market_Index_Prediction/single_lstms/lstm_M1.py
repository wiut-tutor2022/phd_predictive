...
training_set = dataset_train.iloc[:, 12:13].values
...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 12:13].values

dataset_total = pd.concat((dataset_train['M1'], dataset_test['M1']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted M1')
plt.title('Real vs Predicted M1')
plt.xlabel('Months')
plt.ylabel('M1')
plt.legend()
plt.savefig('lstm_M1.png', dpi=1080, format='png')
plt.show()