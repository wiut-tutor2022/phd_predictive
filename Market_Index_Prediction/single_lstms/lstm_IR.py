...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 5:6].values

dataset_total = pd.concat((dataset_train['IR'], dataset_test['IR']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted IR')
plt.title('Real vs Predicted IR')
plt.xlabel('Months')
plt.ylabel('IR')
plt.legend()
plt.savefig('lstm_IR.png', dpi=1080, format='png')
plt.show()