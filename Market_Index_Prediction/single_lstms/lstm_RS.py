...
training_set = dataset_train.iloc[:, 19:20].values
...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 19:20].values

dataset_total = pd.concat((dataset_train['RS'], dataset_test['RS']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted RS')
plt.title('Real vs Predicted RS')
plt.xlabel('Months')
plt.ylabel('RS')
plt.legend()
plt.savefig('lstm_RS.png', dpi=1080, format='png')
plt.show()