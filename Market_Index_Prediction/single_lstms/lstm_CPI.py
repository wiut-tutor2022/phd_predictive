...
mei_lstm_pred = model.fit(X_train,y_train,batch_size=10,epochs=90)
dataset_test = pd.read_csv('Processed_Input_Data_FTSE100_1985_21.csv', header=0, index_col=0)
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 6:7].values

dataset_total = pd.concat((dataset_train['CPI'], dataset_test['CPI']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted CPI')
plt.title('Real vs Predicted CPI')
plt.xlabel('Months')
plt.ylabel('CPI')
plt.legend()
plt.savefig('lstm_CPI.png', dpi=1080, format='png')
plt.show()