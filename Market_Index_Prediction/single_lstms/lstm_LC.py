...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 11:12].values

dataset_total = pd.concat((dataset_train['LC'], dataset_test['LC']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted LC')
plt.title('Real vs Predicted LC')
plt.xlabel('Months')
plt.ylabel('LC')
plt.legend()
plt.savefig('lstm_LC.png', dpi=1080, format='png')
plt.show()