...
training_set = dataset_train.iloc[:, 16:17].values
...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 16:17].values

dataset_total = pd.concat((dataset_train['I'], dataset_test['I']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted I')
plt.title('Real vs Predicted I')
plt.xlabel('Months')
plt.ylabel('I')
plt.legend()
plt.savefig('lstm_I.png', dpi=1080, format='png')
plt.show()