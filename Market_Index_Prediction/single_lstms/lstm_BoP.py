...
training_set = dataset_train.iloc[:, 15:16].values
...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 15:16].values

dataset_total = pd.concat((dataset_train['BoP'], dataset_test['BoP']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted BoP')
plt.title('Real vs Predicted BoP')
plt.xlabel('Months')
plt.ylabel('BoP')
plt.legend()
plt.savefig('lstm_BoP.png', dpi=1080, format='png')
plt.show()