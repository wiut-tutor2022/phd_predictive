...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 2:3].values

dataset_total = pd.concat((dataset_train['GVAph'], dataset_test['GVAph']), axis=0)
...
print('Predicted GVAph:')
# print(predicted_stock_price)
print(predicted_stock_price.transpose())

plt.rcParams["font.family"] = "Times New Roman"
plt.plot(real_stock_price, color='black', label='Real')
plt.plot(predicted_stock_price, color='green', label='Predicted GVAph')
plt.title('Real vs Predicted GVAph')
plt.xlabel('Months')
plt.ylabel('GVAph')
plt.legend()
plt.savefig('lstm_GVAph.png', dpi=1080, format='png')
plt.show()