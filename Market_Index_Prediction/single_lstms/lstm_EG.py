...
real_stock_price = dataset_test.iloc[(len(training_set) - testing_set_size):, 4:5].values

dataset_total = pd.concat((dataset_train['EG'], dataset_test['EG']), axis=0)
...
plt.plot(predicted_stock_price, color='green', label='Predicted EG')
plt.title('Real vs Predicted EG')
plt.xlabel('Months')
plt.ylabel('EG')
plt.legend()
plt.savefig('lstm_EG.png', dpi=1080, format='png')
plt.show()